...
let libs = [["https://unpkg.com/react@16/umd/react.development.js", "text/javascript"], ["https://unpkg.com/react-dom@16/umd/react-dom.development.js", "text/javascript"], ["https://momentjs.com/downloads/moment.js", "text/javascript"], ["https://cdnjs.cloudflare.com/ajax/libs/antd/3.19.0/antd.min.js", "text/javascript"], ["https://cdnjs.cloudflare.com/ajax/libs/antd/3.19.0/antd.min.css", "text/css"]]; //
  ...
  class App extends React.Component {
    constructor(...args) {
      ...
      _defineProperty(this, "state", {
        src: "{{Test|p1={{Test|p2=ddd}}}} {{Test|p1={p2|p}=22}|o3={dd}|d{=p3|ddd}}",
        treeView: null,
        result: "",
        errors: "",
        title: "",
      ...
      _defineProperty(this, "evalResult", async (src = this.state.src, title = this.state.title) => {
        const {
          url,
          params
        } = this.state;
      ...
      _defineProperty(this, "getTemplateSource", async (title = this.state.title) => {
        if (!title) return;
      ...
      _defineProperty(this, "stepIntoTemplate", async () => {
        const {
          selectedNode,
          url,
          src,
        ...
        let titleSrc = src.substring(selectedNode.children[0].start, selectedNode.children[0].end + 1);
        let params = [];
        ...
        try {
          ...
          stepHistory[stepHistory.length - 1].command = src.substring(selectedNode.start, selectedNode.end + 1);
          this.setState({
            title,
      ...
      _defineProperty(this, "navigateInHistory", index => async () => {
        let {
          stepHistory,
          src,
          url
      ...
      _defineProperty(this, "treeNodeOnSelect", (selectedKeys, info) => {
        // jump to template source
        this.setState({
          selectedNode: info.node.props.node
        });
        let {
          start,
          end,
          type
        } = info.node.props.node;

        if (start != undefined && end != undefined) {
          this.setState({
            inputHighlight: [start, end]
          });
        }

        if (type == "root") this.setState({
          inputHighlight: null
        });

        if (type == "template") {
          this.setState({
            stepIntoTemplateButtonDisabled: false
      ...
      _defineProperty(this, "ErrorSection", () => {
        const {
          errors,
          unmatchedBracket
        } = this.state;
        return React.createElement("div", {
          id: "debugger-errors",
          className: "debugger-section"
        }, React.createElement(Title, {
          level: 4
        }, "Errors"), React.createElement("div", {
          id: "debugger-errors-content"
        }, errors && React.createElement(Text, {
          type: "danger"
        }, "Error: ", errors, React.createElement("br", null)), unmatchedBracket.length > 0 && unmatchedBracket.map((b, i) => React.createElement(Text, {
          type: "warning",
          key: i,
          onClick: () => this.setState({
            inputHighlight: [b.start, b.start]
          })
        }, "Warning: unmacthed bracket ", b.value, React.createElement("br", null)))));
      });

      _defineProperty(this, "CallStackSection", () => {
        const {
          stepHistory
        } = this.state;
        ...
        return React.createElement("div", {
          ...
          level: 4,
          type: "secondary"
        }, "Call Stack"), React.createElement("div", {
          className: "non-padding-section"
        }, stepHistory.map((h, i) => React.createElement(React.Fragment, {
          key: i
        }, i > 0 && React.createElement(Icon, {
          type: "right"
        }), React.createElement(Button, {
          type: "link",
          onClick: this.navigateInHistory(i)
        }, h.title)))), stepHistory.slice(0, stepHistory.length - 1).map((h, i) => React.createElement(Typography, null, React.createElement("strong", null, h.title), ": ", h.command)));
      });

      _defineProperty(this, "ParamsTable", () => {
        const {
          params
        } = this.state;
        ...
        return React.createElement("div", null, React.createElement(Title, {
          level: 4,
          type: "secondary"
        }, "Parameters"), React.createElement(Table, {
      ...
      apiParse(src, title, url, (k, t) => {
        if (k == "OK") {
          ...
          if (result.parse && result.parse.parsetree) {
            let ast = result.parse.parsetree["*"] ? getXMLParser()(result.parse.parsetree["*"]) : null;
            nindex = 0;
            ...
            console.log(treeView); // updateFromXML(result.expandtemplates.parsetree, newparams);
          } else {
            // updateFromXML("");
            if (!result.error || result.error.code != "notext") this.setState({
              treeView: null,
              unmatchedBracket: [],
              errors: t
    ...
    generateTreeNode(node) {
      const {
        ...
        id,
        children
      } = node; // type = formatType(type);
      // value = format(value)

      return React.createElement(TreeNode, {
        title: `${node.type ? node.type : ""}${node.type && node.value ? " " : ""}${node.value ? node.value : ""}`,
        key: node.id,
        node: node
      }, node.children.length > 0 && node.children.map(c => this.generateTreeNode(c)));
    }
    ...
    TreeView() {
      ...
      return React.createElement("div", {
        id: "debugger-tree-view"
      }, React.createElement(Title, {
        level: 4,
        type: "secondary"
      }, "Tree View"), React.createElement("div", {
        id: "debugger-tree-view-content"
      }, treeView && React.createElement(Tree, {
    ...
    inputTextWithHighlight() {
      ...
      if (!inputHighlight) return React.createElement(Text, null, src);
      return React.createElement(React.Fragment, null, inputHighlight[0] > 0 && React.createElement(Text, {
        disabled: true
      }, src.substring(0, inputHighlight[0])), React.createElement(Text, null, src.substring(inputHighlight[0], inputHighlight[1] + 1)), inputHighlight[1] < src.length - 1 && React.createElement(Text, {
        disabled: true
    ...
    InputSection() {
      ...
      return React.createElement("div", {
        ...
        level: 4,
        type: "secondary"
      }, "Source", React.createElement(Button, {
        onClick: () => this.setState({
          editIntput: true
        }),
        disabled: editIntput,
        type: "primary",
        icon: "edit"
      })), editIntput ? React.createElement(TextArea, {
        value: src,
    ...
    render() {
      ...
      return React.createElement(Row, null, React.createElement(Col, {
        span: 12
      }, this.InputSection(), this.ErrorSection()), React.createElement(Col, {
        span: 12
      }, React.createElement("div", {
        id: "debugger-debugging-pane",
        className: "debugger-section"
      }, React.createElement(Title, {
        className: "debugger-section-title debugger-title",
        level: 4
      }, "Debugging Pane", React.createElement(Button, {
        ...
        type: "primary",
        icon: "caret-right"
      })), this.CallStackSection(), this.ParamsTable(), React.createElement("div", null, React.createElement("div", {
        id: "debugger-result"
      }, React.createElement(Title, {
        level: 4,
        type: "secondary"
      }, "Result"), React.createElement("div", {
        id: "debugger-result-content"
      }, React.createElement(Paragraph, null, result))), this.TreeView()))));
    }
...
var nindex = 0; // For indexexing AST nodes with unique numbers.

var xindex = 0; // Mapping from nindex to xindex.

var nTox = []; // Root of the AST.

var ast; // Array of things done for undoing.  Each entry is a string or an array of strings.// Constant: timeout interval in ms used in making sequences of API calls.
...
function doPost(url, params, callback) {
  ...
  x.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); // x.setRequestHeader("Content-length", params.length);
...
function apiParse(t, title, url, callback) {
  ...
  args = args + "&text=" + encodeURIComponent(t); //debugNote("Action is "+action);
...
function apiEval(t, title, url, callback) {
  ...
  args = args + "&text=" + encodeURIComponent(t); // args = args + "&frame=" + encodeURIComponent(JSON.stringify({ p1: "hellodd" });
...
async function apiGetSource(title, homePageUrl) {
  let response = await fetch(getUrl(`${homePageUrl}/Template:${title}?action=raw`));
...
function apiGetPage(t, callback) {
  var args = "action=query&format=json&prop=revisions&rvprop=content&titles=" + encodeURIComponent(t);
...
function apiGetTemplateName(t, callback) {
  var args = "action=parse&format=json&prop=templates&contentmodel=wikitext&text=" + encodeURIComponent(t);
  var url = document.getElementById("dt-api").value;
  doPost(url, args, callback);
}
/**
...
function transcludeNoAndOnly(text) {
  ...
  var singleTag = m[2].indexOf("\\/>") > 0; // Certainly have the text prior to the tag

  rc = m[1].replace(reIO, "").replace("</includeonly>", ""); // We have an outer noinclude.  Look for a closing tag, discard the enclosed text,
...
function updateFromXML(x, inheritparams) {
  var i, pname; // First parse the xml and build an AST

  ast = x === "" ? null : getXMLParser()(x); // // Wipe out the global var of previous params and define a new set
  // params = [];
  //
...
function updateParams(params, inheritparams) {
  ...
  var eall = mw.message("debugtemplates-args-eval-all"); // We are going to replace the entire table body and wipe out the existing one

  var new_tbody = document.createElement("tbody"); // There may not be a tbody, so it can be null
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    var row = document.createElement("tr"); // First cell is the set/unset status

    var c = document.createElement("td");
    c.className = "dt-arg-centered"; // Create a toggle-able boolean value

    var span = document.createElement("span");

    if (inheritparams && inheritparams[params[i].name] !== undefined || oldval !== null && oldval.classList.contains("dt-arg-set-yes")) {
    ...
    c.appendChild(span);
    row.appendChild(c); // Then create the parameter name

    c = document.createElement("td");
    c.appendChild(document.createTextNode(params[i].name));
    row.appendChild(c); // The parameter value is a textarea since it can be large, multiline text
    ...
    span.style.width = "95%";
    c.appendChild(span);
    row.appendChild(c); // Then create the eval-all-instances button
    ...
    c.appendChild(span);
    row.appendChild(c); // Ensure the params entry's row field is correct
...
function mapAstToSrc(ast, src) {
  ...
  while (ast_i < templatesAndParams.length) {
    ...
    let {
      ...
      expectedEnd,
      expectedStartLen
    } = getExpectedPattern(curr); // match start
    ...
    while (src_i < src.length) {
      ...
      if (expectedStartLen >= 0 && src.substr(src_i, expectedStartLen) == expectedStart) {
        curr.start = src_i;
    ...
    if (curr.type == "unmatchedBracket" || curr.type == "assignmentSign" || expectedStartLen == 0) {
    ...
    while (src_i < src.length) {
      ...
      if (curr.type == "part" || curr.type == "value") {
        // part unfinished
        if (curr.type == "part" && templatesAndParams[ast_i] && (templatesAndParams[ast_i].type == "name" || templatesAndParams[ast_i].type == "value")) break;
        let prev = stack_ast[curr.type == "part" ? stack_ast.length - 2 : stack_ast.length - 3];
        let prevPattern = getExpectedPattern(prev); // part && template/ argument finished

        if (src.substr(src_i, prevPattern.expectedStartLen) == prevPattern.expectedEnd) {
          curr.end = src_i - 1;
          stack_ast.splice(stack_ast.length - 1, 1); // pop
        ...
        else if (c == "|" && templatesAndParams[ast_i] && templatesAndParams[ast_i].type == "part") {
            curr.end = src_i - 1;
            stack_ast.splice(stack_ast.length - 1, 1); // pop
      ...
      // title
      else if ((curr.type == "template" || curr.type == "tplarg") && !curr.children[0].start && c == "|") {
          curr.children[0].start = curr.start + expectedStartLen;
          curr.children[0].end = src_i - 1;
          break;
        } else if (c == "{") break; // match next
        else if (c == "}") {
            if (expectedStartLen < 0 || src.substr(src_i, expectedStartLen) != expectedEnd) {
              if (templatesAndParams[ast_i] && templatesAndParams[ast_i].type == "unmatchedBracket") break;
              console.log("err");
            }

            curr.end = src_i + expectedStartLen - 1; // title
...
function extractTemplatesAndParams(ast) {
  ...
  while (stack_tree_explore.length > 0) {
    ...
    if (curr.type == "template" || curr.type == "tplarg" || curr.type == "part" || curr.type == "name" || curr.type == "value") templatesAndParams.push(curr);
...
function getExpectedPattern(node) {
  ...
  if ((node.type == "name" || node.type == "value") && node.children.length != 0) {
    expectedStartLen = -1;
  }
...
function paramEval() {
  ...
  setBusy(true); // We need to know our row number

  var rown = row.id.replace(/[^0-9]*/g, ""); // Look through all the parameters displayed

  var instances = document.getElementsByClassName("dt-node-tplarg"); // Start off a chain of individual lookups
