...
function emphasizeTemplate() {
  var outSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-template-out")
  );
  var inSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-template-in")
  );
  var nameSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-template-name")
  );
  var argSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-template-args")
  );
  ...
  for (var i = 0; i < pipes.length; i++) {
    ...
    document
      .getElementById("dt-id-" + pipes[i])
      .classList.toggle("dt-emphasize-template-pipe");
  }
}
...
function emphasizeTplarg() {
  var outSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-tplarg-out")
  );
  var inSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-tplarg-in")
  );
  var nameSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-tplarg-name")
  );
  var argSpan = document.getElementById(
    "dt-id-" + this.getAttribute("dt-emph-tplarg-args")
  );
  ...
  for (var i = 0; i < pipes.length; i++) {
    ...
    document
      .getElementById("dt-id-" + pipes[i])
      .classList.toggle("dt-emphasize-tplarg-pipe");
  }
}
...
function paramEvalNext(i, instances, rown) {
  ...
  while (i < instances.length) {
    ...
    if (params[pindex] && params[pindex].row == rown) {
      ...
      if (txt == "") {
        ...
        window.setTimeout(
          function(txt, n, i, instances, rown) {
            apiEval(txt, function(k, t) {
              var row;
              if (k == "OK") {
                var result = window.JSON.parse(t);
  ...
  if (!continuing) {
    ...
    var lastLastUndo =
      lastUndo.length > 0 ? lastUndo[lastUndo.length - 1] : null;
    if (
      lastLastUndo &&
...
function descendInto(node) {
  ...
  for (i = 0; i < astNode.childNodes.length; i++) {
    ...
    if (
      astNode.childNodes[i].tagName == "part" &&
          ...
          apiEval(args[i], function(k, t) {
            if (k == "OK") {
              ...
              if (apiEvalHasResult(result)) {
                ...
                debugNote(
                  mw.message("debugtemplates-error-arg-eval") + " " + t
                );
                setBusy(false);
              }
            } else {
...
function descendDisplay(node, args) {
  ...
  for (var i = 1; i < node.childNodes.length; i++) {
    if (
      node.childNodes[i].tagName == "part" &&
      node.childNodes[i].childNodes.length == 3
    ) {
      ...
      if (k == "OK") {
        ...
        if (
          ...
          if (tplate["exists"] !== undefined) {
            ...
            debugNote(
              mw.message("debugtemplates-warning-template-not-a-template") +
                " " +
                tname
            );
            setBusy(false);
          ...
          debugNote(
            mw.message("debugtemplates-warning-template-not-found") +
              " " +
              args[0]
          );
          setBusy(false);
      ...
      if (result.query && result.query.pages) {
        for (var p in result.query.pages) {
          ...
          if (pg.revisions && pg.revisions[0]) {
            ...
            debugNote(
              mw.message("debugtemplates-error-template-page") + " " + tname
            );
            setBusy(false);
          }
        }
...
function toggleSetButton() {
  ...
  if (abody && abody[0]) {
    ...
    for (i = 0; i < abody.rows.length; i++) {
      if (
        abody.rows[i].cells[2].firstChild.classList.contains("dt-arg-set-no")
      ) {
        setgoal = "on";
        break;
      }
...
function init() {
  ...
  document
    .getElementById("dt-api")
    .addEventListener("change", apiHandler, false);
  document
    .getElementById("dt-title")
    .addEventListener("change", titleHandler, false);
  ...
  document
    .getElementById("dt-eval")
    .addEventListener("click", evalAllButton, false);
  document
    .getElementById("dt-args-set-toggle")
    .addEventListener("click", toggleSetButton, false);
  document
    .getElementById("dt-args-value-clear")
    .addEventListener("click", clearAllButton, false);

  document
    .getElementById("dt-radio-select")
    .addEventListener("click", clickMode, false);
  document
    .getElementById("dt-radio-eval")
    .addEventListener("click", clickMode, false);
  document
    .getElementById("dt-radio-descend")
    .addEventListener("click", clickMode, false);
  ...
  document
    .getElementById("dt-input")
    .addEventListener("mouseup", resizeArgTable, false);
